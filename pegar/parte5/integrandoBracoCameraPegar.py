...
def pegar():
    global alturaSobe, posicaoFrente
    ...
    indoFrente = True
    posicaoFrenteSetPoint = 68
    
    avancar = True
    avancarSetPoint = 100
    
    while descendo:
        if alturaSobe > desceuSetPoint:
            alturaSobe = alturaSobe - 1
            enviarMensagem("sobe" + str(alturaSobe), 0.05)
        else:
            descendo = False
    
    while indoFrente:
        if posicaoFrente < posicaoFrenteSetPoint:
            posicaoFrente = posicaoFrente + 1
            enviarMensagem("frente" + str(posicaoFrente), 0.05)
        else:
            indoFrente = False
    
    while avancar:
        if alturaSobe < avancarSetPoint:
            alturaSobe = alturaSobe + 1
            enviarMensagem("sobe" + str(alturaSobe), 0.05)
        else:
            avancar = False
    
    enviarMensagem("garra5", 2)
    subindo = True
    
    posicaoFrenteSetPoint = 30
    indoFrente = True
    ...
    while subindo:
        if alturaSobe < subirSetPoint:
            alturaSobe = alturaSobe + 1
            enviarMensagem("sobe" + str(alturaSobe), 0.05)
        else:
            subindo = False
    enviarMensagem("sobe" + str(subirSetPoint), 2)
    print("Peguei")
